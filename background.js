...
globalThis.NOTIFICATION_ICON_DATA_URL = NOTIFICATION_ICON_DATA_URL;
...
function normalizeSettings(settings = {}) {
  ...
  return {
    ...
    maxContinuations: Math.round(clampNumber(settings.maxContinuations, DEFAULTS.maxContinuations, 1, 50)),
    ...
    contextCapacityTokens: Math.round(clampNumber(settings.contextCapacityTokens, DEFAULTS.contextCapacityTokens, 16000, 1000000)),
    contextThresholdPercent: clampNumber(settings.contextThresholdPercent, DEFAULTS.contextThresholdPercent, 50, 98),
...
async function saveState(state) {
  await chrome.storage.session.set({ [SESSION_KEY]: { ...state, savedAt: Date.now() } });
}

async function notify(state, title, message, idSuffix = "event") {
  if (!state?.settings?.notificationsEnabled) return;
  try {
    await chrome.notifications.create(`autoprompter-${idSuffix}-${Date.now()}`, {
      type: "basic",
...
async function stopScheduler(reason = "Stopped", error = "", closeWorkers = true, notifyUser = false) {
  ...
  await Promise.all(tabIds.map(async tabId => {
    try { await chrome.tabs.sendMessage(tabId, { type: "CANCEL_CHAT_JOB", token: stopped.token }); } catch { /* best effort */ }
  }));
...
async function saveSuccessorToCatalog(chat, parentId = "") {
  ...
  await chrome.storage.local.set({
    [CATALOG_KEY]: [successorEntry, ...catalog.filter(item => item.id !== chat.id)],
...
async function releaseInitialBatch(state, force = false) {
  ...
  await saveState(state);
  const results = await Promise.all(readyIndexes.map(index => sendChatJob(state, index, { initialBatch: true, batchPrepared: true })));
  for (let offset = 0; offset < readyIndexes.length; offset += 1) {
    const chat = state.chats[readyIndexes[offset]];
    if (results[offset]) chat.initialJobPending = false;
    else {
      chat.jobDispatched = false;
...
async function launchAllWorkers(state) {
  ...
  updateOverallStatus(state);
  await saveState(state);
  await Promise.all(indexes.filter(index => Number.isInteger(state.chats[index].workerTabId)).map(async index => {
    const chat = state.chats[index];
    try {
...
async function startScheduler(chats, settings, mode = "work") {
  ...
  if (normalizedChats.length > MAX_CONCURRENT_CHATS) throw new Error(`Select at most ${MAX_CONCURRENT_CHATS} chats for one concurrent run.`);
...
async function beginSuccessor(state, index, message) {
  ...
  if (!verified && !bestEffortAllowed) return failChatWorker(state, index, `Continuity handoff required: ${reason}`);
  ...
  chat.pendingSuccessor = {
    ...
    checkpoint,
    reason,
    prompt: verified ? buildSuccessorPrompt(chatSettings, chat, checkpoint, reason) : buildFreshStartPrompt(chatSettings, chat, reason),
...
async function successorCreated(message, sender) {
  ...
  await notify(
    state,
    "Successor chat ready",
    successor.lastCheckpoint ? `${successor.title} · checkpoint ${successor.lastCheckpoint}` : `${successor.title} · best-effort fresh start`,
  ...
  const operation = async () => {
    switch (message.type) {
      ...
      case "CONTENT_READY": {
        const state = await loadState();
        const index = findChatIndexByTab(state, sender?.tab?.id);
        return state?.running && index >= 0 ? markContentReady(state, index) : publicState(state);
      }
      case "JOB_STATUS":
